...
A migration that retypes a persisted field must call `crate::session::backup_before_repair` before rewriting, so the build that predates the new shape still has bytes it can read. What that build does with a row it cannot decode, and what the restore point is worth to a user who forces a downgrade, is covered under [Downgrading](../installation.md#downgrading).
