...
An explicit pin or fork that migration left unqualified also resumes, against the store current configuration resolves, and no capture can qualify it while the pin is armed. An agent that publishes its own conversation consumes the pin at launch, so the row keeps the conversation id with the binding the launch recorded, and a failed resume leaves the next restart on the automatic path. An agent without such a publisher keeps the pin armed and repeats that failure. `set-session-id` is the only way to assert a store without launching. Pi and OMP still need the exact transcript, recovered with `--store`. To assert a store explicitly:
