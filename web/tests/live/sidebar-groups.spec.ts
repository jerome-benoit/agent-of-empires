...
base.describe("sidebar repo groups (#1220)", () => {
  base(
    "two repos render as two groups, both rows visible",
    async ({ page }, testInfo) => {
  ...
  base(
    "filter input narrows visible groups + rows by repo name",
    async ({ page }, testInfo) => {
      const serve = await spawnAoeServe({
        authMode: "none",
        workerIndex: testInfo.workerIndex,
  ...
  base(
    "group header chevron toggles aria-expanded and hides rows",
    async ({ page }, testInfo) => {
      const serve = await spawnAoeServe({
        authMode: "none",
        workerIndex: testInfo.workerIndex,
