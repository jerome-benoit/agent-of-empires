...
base(
  "DirectoryBrowser: home -> projects -> repo-a -> persisted last dir",
  async ({ page }, testInfo) => {
    ...
    try {
      ...
      await expect(
        page.getByRole("option").filter({ hasText: "repo-a" }),
      ).toBeVisible({
        timeout: 10_000,
      });
      // `projects` no longer appears as an entry: we're inside it.
      await expect(
        page.getByRole("option").filter({ hasText: "projects" }),
      ).toHaveCount(0);
    } finally {
      await serve.stop();
    }
...
base(
  "DirectoryBrowser: parent-dir row navigates up one level",
  async ({ page }, testInfo) => {
    const serve = await spawnAoeServe({
      authMode: "none",
      workerIndex: testInfo.workerIndex,
    ...
    try {
      ...
      await page
        .getByRole("option")
        .filter({ hasText: "(parent directory)" })
        .click();
      await expect(
        page.getByRole("option").filter({ hasText: "nested" }),
      ).toBeVisible({
        timeout: 5_000,
      });
    } finally {
