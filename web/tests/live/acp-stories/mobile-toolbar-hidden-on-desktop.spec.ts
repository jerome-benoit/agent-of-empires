...
base(
  "mobile toolbar is hidden on a desktop viewport",
  async ({ page }, testInfo) => {
    ...
    try {
      ...
      if (!seeded)
        throw new Error("seeded session 'story-toolbar-desktop' missing");
      ...
      await expect(page.getByRole("button", { name: "Arrow up" })).toHaveCount(
        0,
      );
      await expect(
