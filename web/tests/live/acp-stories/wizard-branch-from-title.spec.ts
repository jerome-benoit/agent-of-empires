...
base(
  "wizard derives the branch from the title on the Review step",
  async ({ page }, testInfo) => {
    ...
    try {
      ...
      const titleInput = page
        .locator('input[placeholder="Auto-generated"]')
        .first();
      await titleInput.fill("autogen-branch-here");
      await titleInput.blur();

      // Title + branch rows both show "autogen-branch-here".
      await expect(page.getByText("autogen-branch-here").first()).toBeVisible({
        timeout: 10_000,
      });
      const occurrences = await page.getByText("autogen-branch-here").count();
