...
base(
  "ModePicker switches the structured view mode",
  async ({ page }, testInfo) => {
    ...
    try {
      ...
      if (!seeded)
        throw new Error("seeded session 'story-mode-picker' missing");
      ...
      const trigger = page
        ...
        .filter({
          has: page.locator(":scope > span", {
            hasText: /^(Default|Plan|Accept|Bypass)$/,
          }),
        })
        .first();
...
base(
  ...
  async ({ page }, testInfo) => {
    ...
    try {
      ...
      if (!seeded)
        throw new Error("seeded session 'story-mode-opencode' missing");
      const sessionId = seeded.id;
      await enableStructuredViewAndWait(serve.baseUrl, sessionId);
      const spawnRes = await fetch(
        `${serve.baseUrl}/api/sessions/${sessionId}/acp/spawn`,
      ...
      const trigger = page
        .locator("button")
        .filter({
          has: page.locator(":scope > span", { hasText: /^(Build|Plan)$/ }),
        })
        .first();
      await expect(trigger).toBeVisible({ timeout: 10_000 });
