...
// "aoe-acp-last-tool" on every submit. Reopen the wizard later and
...
base(
  "wizard remembers the last-picked agent after reload",
  async ({ page }, testInfo) => {
    ...
    try {
      ...
      // The key is `aoe-acp-last-tool` (LAST_USED_TOOL_KEY in
      ...
      const stored = await page.evaluate(() =>
        localStorage.getItem("aoe-acp-last-tool"),
      );
