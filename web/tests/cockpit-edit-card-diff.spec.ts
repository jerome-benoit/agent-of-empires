...
// Mocked render of the cockpit Edit tool card (#1768).
//
...
// spec drives a single `ToolCallStarted` edit frame over the cockpit
...
async function setup(page: Page) {
    ...
    return r.fulfill({
      json: {
        sessions: [
          {
            id: SESSION_ID,
            title: "cockpit-edit-card",
            project_path: "/tmp/cockpit-edit-card",
            group_path: "/tmp",
            tool: "claude",
            status: "Running",
            yolo_mode: false,
  ...
  // Cockpit REST endpoints (replay/snapshot/prompt): empty is fine, the
  // tool frame arrives over the WebSocket below.
  await page.route("**/api/sessions/*/cockpit/**", (r) =>
    r.fulfill({ json: {} }),
  );

  // Terminal WS (only opened outside cockpit mode): swallow it.
  await page.routeWebSocket(/\/sessions\/[^/]+\/ws(\?|$)/, () => {
    // no-op
  });
  // Cockpit WS: push one edit tool frame so the card renders.
  await page.routeWebSocket(/\/sessions\/[^/]+\/cockpit\/ws/, (ws) => {
    ws.send(JSON.stringify(editFrame()));
  });
}

test("cockpit edit card renders diffPair output (chip + StringDiff)", async ({
  page,
}) => {
  await setup(page);
  await page.goto("/");
  await expect(page.locator("header")).toBeVisible();
  await clickSidebarSession(page, "cockpit-edit-card");
