...
mod tests {
    ...
    mod resume_fallback {
        ...
        fn clear_session_id_on_disk_skips_on_cas_mismatch() {
            ...
            let storage =
                crate::session::storage::Storage::new_for_test("cas-clear-mismatch").unwrap();
            let mut inst = Instance::new("title", "/tmp/x");
            inst.agent_session_id = Some("peer-wrote".to_string());
            let id = inst.id.clone();
            let xs = vec![inst];
        ...
        fn reset_resume_intent_to_default_auto_promotes_cleared() {
            ...
            let storage = crate::session::storage::Storage::new_for_test("intent-promote").unwrap();
            let mut inst = Instance::new("title", "/tmp/x");
        ...
        fn reset_resume_intent_to_default_skips_on_cas_mismatch() {
            ...
            let storage =
                crate::session::storage::Storage::new_for_test("intent-mismatch").unwrap();
            let mut inst = Instance::new("title", "/tmp/x");
        ...
        fn cascade_tier1_downgrades_use_pin_to_default_when_dead() {
            ...
            let storage =
                crate::session::storage::Storage::new_for_test("cascade-use-downgrade").unwrap();
            let mut inst = Instance::new("title", "/tmp/x");
